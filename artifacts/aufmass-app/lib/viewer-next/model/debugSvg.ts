...
export function renderDebugSvg(model: ViewerModel): string {
  ...
  return [
    ...
    ...(model.permanentDimensions.length?.segments ?? []).map(segment => dimensionLine("length", segment)),
    ...(model.permanentDimensions.depth?.segments ?? []).map(segment => dimensionLine("depth", segment)),
    ...(model.permanentDimensions.eaveHeight?.segments ?? []).map(segment => dimensionLine("eave-height", segment)),
    ...(model.permanentDimensions.length ? [`    <text data-dimension-label="length">${escapeXml(model.permanentDimensions.length.label)}</text>`] : []),
    ...(model.permanentDimensions.depth ? [`    <text data-dimension-label="depth">${escapeXml(model.permanentDimensions.depth.label)}</text>`] : []),
    ...(model.permanentDimensions.eaveHeight ? [`    <text data-dimension-label="eave-height">${escapeXml(model.permanentDimensions.eaveHeight.label)}</text>`] : []),
    "  </g>",
    "</svg>",
  ].join("\n");
}
