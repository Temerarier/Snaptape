...
- Alternative for one-off CLIs: agent shell commands are hard-capped at 5 minutes and kill the foreground process too (a ~6.5 min regression run died mid-extraction and left projects stuck in "processing"). Run the command as a temporary console workflow (`configureWorkflow` + poll `getWorkflowStatus`), then remove the workflow. The user's own terminal has no such cap — `pnpm regression` runs fine for them foreground.
